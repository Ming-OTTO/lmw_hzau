...
class MixVisionTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.init_weight()  # backbones初始化权重

    def init_weight(self):
        if self.pretrained is not None:
            utils.load_pretrained_model(self, self.pretrained)  # 加载部分预训练模型
        else:
            self.apply(self._init_weights)

    def _init_weights(self, m):  # 使用了一种常见的权重初始化策略，称为“截断正态分布”（Truncated Normal Distribution）
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight)
            if isinstance(m, nn.Linear) and m.bias is not None:
                zeros_(m.bias)
