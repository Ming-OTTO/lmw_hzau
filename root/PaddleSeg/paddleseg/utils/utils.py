...
def resume(model, optimizer, resume_model):
    if resume_model is not None:
        ...
        if os.path.exists(resume_model):
            resume_model = os.path.normpath(resume_model)  # normpath是规范化路径
            ckpt_path = os.path.join(resume_model, 'model.pdparams')
            para_state_dict = paddle.load(ckpt_path)
            ckpt_path = os.path.join(resume_model, 'model.pdopt')
            opti_state_dict = paddle.load(ckpt_path)  # 加载优化器参数：包括学习率（学习计划）、动量、权重衰减等 last_epoch=195700，last_lr=0.00001
            model.set_state_dict(para_state_dict)
            optimizer.set_state_dict(opti_state_dict)  # 将优化器的参数设置为加载的参数，覆盖了原来的参数
