...
def parse_args():
    ...
    parser.add_argument(
        '--split', help='', nargs=3, type=float, default=[0.7, 0.2, 0.1])
    parser.add_argument(
        '--separator',
        dest='separator',
...
def generate_list(args):
    ...
    image_files = get_files(image_dir, args.format[1], args.postfix[1])
    ...
    state = np.random.get_state()   # 保存随机数种子
    np.random.shuffle(image_files)
    np.random.set_state(state)   # 恢复随机数种子
    np.random.shuffle(label_files)

    start = 0
    num_split = len(args.split)
    dataset_name = ['train_list', 'val_list', 'test_list']
