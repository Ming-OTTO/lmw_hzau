...
import pandas as pd
from PIL import Image  # 新增库用于处理PNG图像
...
class HyperspectralProcessor:
    ...
    def load_binary_mask(self, mask_path):
        """
        加载二值掩膜图像
        ...
        """
        try:
            print(f"正在加载二值掩膜: {mask_path}")

            # 检查文件扩展名
            file_ext = os.path.splitext(mask_path)[1].lower()

            if file_ext in ['.hdr', '.dat', '.img']:
                # 尝试作为ENVI格式加载
                ...
                # 使用PIL加载图像
                img = Image.open(mask_path).convert('L')  # 转换为灰度图
                mask = np.array(img)
    ...
    def extract_roi_reflectance(self, binary_mask, output_csv=None, stats=['mean', 'std', 'min', 'max']):
        """
        根据二值掩膜提取感兴趣区域的反射率，并导出统计结果到CSV

        参数:
        binary_mask: 二值掩膜数组 (True表示感兴趣区域)
        output_csv: 输出CSV文件路径，如果为None则不导出
        stats: 需要计算的统计量，可选值: 'mean', 'std', 'min', 'max', 'median'
        ...
        # 检查掩膜维度是否与反射率数据匹配
        if binary_mask.shape != self.reflectance.shape[:2]:
            raise ValueError(f"掩膜形状 ({binary_mask.shape}) 与反射率数据 ({self.reflectance.shape[:2]}) 不匹配")

        # 提取掩膜区域的所有像元
        print("正在提取感兴趣区域的反射率...")
        roi_pixels = self.reflectance[binary_mask]

        print(f"提取了 {roi_pixels.shape[0]} 个像元")

        # 计算统计量
        results = {}

        if 'mean' in stats:
            results['mean'] = np.mean(roi_pixels, axis=0)

        if 'std' in stats:
            results['std'] = np.std(roi_pixels, axis=0)

        if 'min' in stats:
            results['min'] = np.min(roi_pixels, axis=0)

        if 'max' in stats:
            results['max'] = np.max(roi_pixels, axis=0)

        if 'median' in stats:
            results['median'] = np.median(roi_pixels, axis=0)

        # 创建DataFrame
        if self.wavelengths is not None and len(self.wavelengths) == results['mean'].shape[0]:
            # 使用波长作为列名
            columns = [f"{stat}_{wavelength:.2f}nm" for wavelength in self.wavelengths for stat in results.keys()]
            data = np.hstack([results[stat].reshape(1, -1) for stat in results.keys()])
            df = pd.DataFrame(data, columns=columns)
        else:
            # 使用波段索引作为列名
            columns = [f"{stat}_band_{i}" for i in range(results['mean'].shape[0]) for stat in results.keys()]
            data = np.hstack([results[stat].reshape(1, -1) for stat in results.keys()])
            df = pd.DataFrame(data, columns=columns)
        ...
        if output_csv:
            ...
            print(f"正在导出统计结果到: {output_csv}")
            df.to_csv(output_csv, index=False)
            print(f"统计结果已保存至: {output_csv}")

        return df

    def plot_roi_spectrum(self, binary_mask, ax=None, label=None, title=None):
        """
        绘制感兴趣区域的平均光谱曲线
        ...
        # 绘制平均光谱
        ax.plot(self.wavelengths, mean_spectrum, label=label or 'ROI平均光谱', color='blue')
        ...
        ax.set_xlabel('波长 (nm)')
        ax.set_ylabel('反射率')
        ax.set_title(title or '感兴趣区域平均反射率光谱')
        ax.grid(True, linestyle='--', alpha=0.7)
...
if __name__ == "__main__":
    ...
    # 二值掩膜路径 (PNG格式)
    binary_mask_path = r"C:\Users\hzauc\Desktop\1\20250709064646.png"

    # 文件格式，可选'envi'或'manual'
    file_format = 'envi'

    # 输出CSV路径
    output_csv = r"H:\lmw_python_pr\高光谱\output\reflectance_stats.csv"
    ...
    try:
        ...
        # 加载二值掩膜
        binary_mask = processor.load_binary_mask(binary_mask_path)

        if binary_mask is not None:
            # 提取感兴趣区域的反射率并导出统计结果
            stats_df = processor.extract_roi_reflectance(
                binary_mask,
                output_csv=output_csv,
                stats=['mean', 'std', 'min', 'max', 'median']
            )

            print(f"统计结果形状: {stats_df.shape}")
            print("统计结果示例:")
            print(stats_df.head())

            # 绘制感兴趣区域的平均光谱
            processor.plot_roi_spectrum(binary_mask, title="感兴趣区域平均反射率光谱")

            # 提取并绘制特定像元的光谱
            row, col = 100, 100
            processor.plot_spectrum(row, col, title="单个像元反射率光谱")
